/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2012-11-18 23:25:33 +0100 (dim., 18 nov. 2012) $
 * $Revision: 17726 $
 *
...
public class FileManager {
  ...
  String getFileTypeName(String fileName) {
    ...
    if (br instanceof ZipInputStream) {
      ...
      return viewer.getModelAdapter().getFileTypeName(
          ZipUtil.getBufferedReaderForString(zipDirectory));
    }
  ...
  Object createAtomSetCollectionFromString(String strModel,
    ...
    FileReader fileReader = new FileReader("string", "string", "string", null,
        ZipUtil.getBufferedReaderForString(strModel), htParams, isAppend);
    fileReader.run();
    if (fnames != null)
      setFileInfo(fnames);
  ...
  Object getUnzippedBufferedReaderOrErrorMessageFromName(
    ...
    if (info != null) {
      ...
      if (info[2] != null) {
        ...
        return ZipUtil.getBufferedReaderForString(s);
      }
      // continuing...
    ...
    try {
      ...
      if (ZipUtil.isGzip(bis)) {
        ...
        return ZipUtil.getBufferedReaderForString(doc.getAllData("Molecule", "Input")
            .toString());
      } else {
      ...
      if (asInputStream)
        return bis;
      return ZipUtil.getInputStreamReader(bis);
    } catch (Exception ioe) {
